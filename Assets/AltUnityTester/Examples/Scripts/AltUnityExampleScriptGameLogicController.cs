...
public class AltUnityExampleScriptGameLogicController : MonoBehaviour {
    public GameObject starPrefab;
    public Camera cameraControlled;
    public AltUnityExampleScriptStarCounter starCounter;
        ...
        if (Input.GetMouseButtonDown(0))
        {
            ...
            if (Physics.Raycast(ray,out hit))
            {
                if (hit.transform.gameObject.name == "Floor")
                {
                    ...
                    star.GetComponent<AltUnityExampleScriptStarController>().starCounter = starCounter;
