...
namespace Utility
{
  public static class PlayerPrefs
  {
    ...
    private static byte[] bytes =
      ASCIIEncoding.ASCII.GetBytes("sk39" + SystemInfo.deviceUniqueIdentifier.Substring(0, 4));
      ...
      else if (File.Exists(fileName))
      {
        ...
        serializedInput = fileReader.ReadToEnd();
			...
			if(UnityEngine.PlayerPrefs.HasKey("encryptedData")) {
				...
				serializedInput =
 (securityModeEnabled?Decrypt(UnityEngine.PlayerPrefs.GetString("data")):UnityEngine.PlayerPrefs.GetString("data"));
			}
    ...
    private static void Deserialize()
    {
      ...
      foreach (string parameter in parameters)
      {
        ...
        playerPrefsHashtable.Add(DeEscapeNonSeperators(parameterContent[0], seperators),
          GetTypeValue(parameterContent[2], DeEscapeNonSeperators(parameterContent[1], seperators)));

        if (parameterContent.Length > 3)
        {
          UnityEngine.Debug.LogWarning("PlayerPrefs::Deserialize() parameterContent has " + parameterContent.Length +
                                       " elements");
        }
      }
    }
